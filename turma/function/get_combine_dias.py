...
def __get_combined_dias(possibilidades_por_professor_e_dia, dia_da_semana, turmas_by_turno,
                        periodos_ordens, disciplinas) -> List[dict]:
    ...
    for current in possibilidades_por_professor_e_dia:
        ...
        for indexes in indexes_groups:
            ...
            result = __get_result(to_combine, dia_da_semana, turmas_by_turno, periodos_ordens, disciplinas)
...
def __get_result(to_combine, dia_da_semana, turmas_by_turno, periodos_ordens, disciplinas) -> dict:
    result_turma: dict = {}
    for turma_by_turno in turmas_by_turno:
        ...
        result_turma[turma_by_turno] = {'cells': result_sector, 'quantidade_periodos_alocados':
            __get_quantity_periodos_alocados(result_sector, turmas_by_turno, disciplinas)}
...
def __get_quantity_periodos_alocados(cells, turmas_by_turno: dict, disciplinas) -> dict:
    quantidade_periodos_map: dict = {}

    for disciplina in disciplinas:
        filtered = [x for x in cells if ' - ' + disciplina in x['allocated']]
        if filtered:
            quantidade_periodos_map[disciplina] = len(filtered)

    return quantidade_periodos_map